...
<form class="flex @if($responsive)) md:flex-row flex-col @else flex-row @endif items-center gap-2 mb-0" id="like-form-{{ $id }}" method="POST"
    action="/blog/{{ $id }}/like">
    @csrf
    ...
    <div class="flex flex-row items-center gap-2">
        {{ $likes }}
        @if ($liked === 'like')
            <button type="button" onclick="like(event, {{ $liked }})" @guest disabled @endguest>
    ...
    <div class="flex md:flex-row flex-row-reverse items-center gap-2">
        @if ($liked === 'dislike')
            <button onclick="dislike(event, {{ $liked }})" @guest disabled @endguest>
                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-thumb-down-filled"
                    width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                    fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path
