<div id="comment_{{ $comment->id }}"
    ...
    @if (isset($comment->user->profile_picture) & ($comment->user->profile_picture != null))
        <div class="w-8 h-8 bg-black rounded-full text-white flex justify-center items-center">
            @if (($comment->user->auth_provider != 'google') & ($comment->user->auth_provider != 'github'))
                <img src="{{ asset('storage/' . $comment->user->profile_picture) }}"
                    alt="{{ strtoupper($comment->user->username[0]) }}" class="object-fit rounded-full w-full h-full">
            @else
                <img src="{{ $comment->user->profile_picture }}" alt="{{ strtoupper($comment->user->username[0]) }}"
                    class="object-fit rounded-full w-full h-full">
            @endif
        </div>
    @else
        <div class="w-8 h-8 bg-black flex justify-center items-center text-white text-4xl rounded-full">
            {{ strtoupper($username[0]) }}
        </div>
    @endif
    <div class="w-fit ml-4">
        <a href="/user/{{ $comment->user->id }}" class="font-semibold hover:underline underline-offset-2">
            {{ '@' . $comment->user->username }}</a>
        <div class="mt-2">{{ $comment->body }}</div>
    </div>
    @auth
        @if (!$isResponse)
            <button id="answer_{{ $comment->id }}"
                onclick="selectParent({{ $comment->id }}, '{{ $comment->user->username }}')"
