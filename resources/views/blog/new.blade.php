...
<div class="w-full min-h-screen flex justify-start flex-col items-center px-12 py-8">
    <form class="max-w-5xl w-full mx-auto flex flex-col justify-start items-center gap-3" method="POST"
        ...
        <textarea style="resize:none" type="text" name="subtitle" id="subtitle"
            class="w-full text-lg px-2 py-2 font-base focus:outline-none" placeholder="Subtitle" maxlength="100" rows="1">{{ old('subtitle') }}</textarea>
        @error('subtitle')
            <div class="text-red-500">{{ $message }}</div>
        @enderror
        <label for="image"
        ...
        <trix-editor
            class="w-full [&>*>a]:text-blue-600 [&>*>a]:underline [&>*>a]:underline-offset-2 [&>h1]:font-semibold [&>h1]:text-2xl [&>h1]:mt-4 [&>h1]:mb-2 [&>h1]:w-full [&>h1]:border-b"
